<?php
/* File: review.php - saves one rating and comment for an enrolled course. */
...
$sql = "SELECT c.course_id, c.course_name FROM enrollments e
...
if (!$course) {
    setFlash('error', 'Only an enrolled user can review this course.');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($rating < 1 || $rating > 5 || $comment == '') {
        ...
        $check = $pdo->prepare('SELECT review_id FROM reviews WHERE user_id = ? AND course_id = ?');
        $check->execute(array($user['user_id'], $courseId));
        $oldReview = $check->fetch();
        if ($oldReview) {
            $statement = $pdo->prepare('UPDATE reviews SET rating = ?, comment = ?, created_at = CURRENT_TIMESTAMP WHERE review_id = ?');
            $statement->execute(array($rating, $comment, $oldReview['review_id']));
        } else {
            $statement = $pdo->prepare('INSERT INTO reviews (user_id, course_id, rating, comment) VALUES (?, ?, ?, ?)');
