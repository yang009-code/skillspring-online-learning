<?php
/* File: my-courses.php - lists courses purchased by the current user. */
...
$sql = "SELECT e.*, c.course_name, c.short_description, c.image,
        ...
        LEFT JOIN progress p ON p.lesson_id = l.lesson_id
             AND p.user_id = e.user_id AND p.completed = 1
        WHERE e.user_id = ?
...
<?php foreach ($enrollments as $enrollment) { ?>
    ...
    <article class="card course-card">
        <img src="<?= BASE_URL ?>/<?= e($enrollment['image']) ?>" alt="<?= e($enrollment['course_name']) ?> course cover">
        <h2><?= e($enrollment['course_name']) ?></h2>
        <p><?= e($enrollment['short_description']) ?></p>
        <p><strong>Access:</strong> <?= e($enrollment['access_plan']) ?></p>
        <p><strong>Expiry:</strong> <?= $enrollment['expiry_date'] ? e($enrollment['expiry_date']) : 'No expiry' ?></p>
        <div class="progress-track" aria-label="Course progress">
            <div class="progress-bar" style="width: <?= $progressPercent ?>%"></div>
        </div>
        <p><?= $progressPercent ?>% complete</p>
        <div class="button-row">
            <a class="btn" href="<?= BASE_URL ?>/learn.php?id=<?= (int)$enrollment['course_id'] ?>">Open Lessons</a>
            <a class="btn secondary" href="<?= BASE_URL ?>/review.php?id=<?= (int)$enrollment['course_id'] ?>">Review</a>
        </div>
    </article>
<?php } ?>
</section>
<?php if (!$enrollments) { ?>
<section class="card"><p>You have not purchased any courses.</p><a class="btn" href="<?= BASE_URL ?>/courses.php">Browse Courses</a></section>
<?php } ?>
