...
if ($pdo) {
    if ($category != '') {
        $sql = "SELECT * FROM courses WHERE status = 'active' AND category = ? ORDER BY course_name";
...
<section class="card">
    ...
    <p>There are 20 course records. Each course has an access plan and a support plan.</p>
    <form class="search-bar" action="<?= BASE_URL ?>/search.php" method="get">
        <input type="search" name="q" placeholder="Search by title, category, instructor or keyword" required>
...
<section class="card-grid section-title">
    ...
    <article class="card course-card">
        ...
        <a class="btn" href="<?= BASE_URL ?>/course-details.php?id=<?= (int)$course['course_id'] ?>">Details and Options</a>
    </article>
    <?php } ?>
</section>

<?php if (!$courses) { ?><section class="card"><p>No courses were found.</p></section><?php } ?>
